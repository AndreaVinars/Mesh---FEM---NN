...
3. **Homogenization (Post-Processing):** 
   ...
   - The Effective Stiffness Matrix ($\mathbf{C}_{eff}$) is identified ($\bar{\boldsymbol{\sigma}} = \mathbf{C}{eff} \cdot \bar{\boldsymbol{\varepsilon}}$), from which the **Effective Young's Modulus ($E{eff}$)** is extracted.
